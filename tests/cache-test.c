...
// Запись данных в кэш.
gpointer data_writer( gpointer data )
{
...
gpointer data_reader( gpointer data )
{
  ...
  g_message( "thread %d: hits: number = %d time = %.3lf ms/req, misses: number = %d time = %.3lf ms/req", thread_id, hit, ( 1000.0 * hit_time ) / hit, miss, ( 1000.0 * miss_time ) / miss );
...
int main( int argc, char **argv )
{
  ...
  if( small_size > 1024 * 1024 ) small_size = 1024 * 1024;
  if( big_size > 1024 * 1024 ) big_size = 1024 * 1024;
  ...
  for( i = 0; i < n_threads; i++ ) g_free( buffers[i] );
  for( i = 0; i < n_patterns; i++ ) g_free( patterns[i] );
  g_free( buffers );
  g_free( patterns );
